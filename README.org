...
well as in =magit-blame-read-only-map= as well as ~M-d~ to
...
bindings.  You need to turn the =difftastic-bindings-mode= off and on again to
apply the changes.
...
demonstrate how to achieve roughly partial effect as
