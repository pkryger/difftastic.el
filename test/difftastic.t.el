...
")
    ...
    (should (equal (cons 'rline-num-end (difftastic--chunk-file-at-point t))
                   '(rline-num-end "foo" nil right)))
    (goto-char (compat-call pos-bol)) ; Since Emacs-29
    (should (equal (cons 'bol (difftastic--chunk-file-at-point))
                   '(bol "foo" 7 left)))
    (should (equal (cons 'rbol (difftastic--chunk-file-at-point t))
                   '(rbol "foo" nil right)))
    (goto-char (compat-call pos-eol)) ; Since Emacs-29
    (should (equal (cons 'eol (difftastic--chunk-file-at-point))
                   '(eol "foo" 7 left)))
    (should (equal (cons 'reol (difftastic--chunk-file-at-point t))
                   '(reol "foo" nil right)))
...
")
    ...
    (should (equal (cons 'rline-num-end (difftastic--chunk-file-at-point t))
                   '(rline-num-end "foo" 98 right)))
    (re-search-backward (rx "."))
    (should (equal (cons 'line-num-beg (difftastic--chunk-file-at-point))
                   '(line-num-beg "foo" nil left)))
    (should (equal (cons 'rline-num-beg (difftastic--chunk-file-at-point t))
                   '(rline-num-beg "foo" 98 right)))
    (goto-char (compat-call pos-bol)) ; Since Emacs-29
    (should (equal (cons 'bol (difftastic--chunk-file-at-point))
                   '(bol "foo" nil left)))
    (should (equal (cons 'rbol (difftastic--chunk-file-at-point t))
...
(ert-deftest difftastic-diff-visit-worktree-file:basic ()
  (mocklet (((difftastic--chunk-file-at-point t) => "test-chunk-file")
            ((difftastic--diff-visit-file
              "test-chunk-file" #'pop-to-buffer-same-window)))
